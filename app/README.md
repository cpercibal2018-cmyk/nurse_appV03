...
- `hr.admin@aigh.sa` → HR_ADMIN (onboarding, org structure, credentials). The mock derives the role from the e-mail, and `hr` is tested **before** `admin` — testing `admin` first signed this account in as SYSTEM_ADMIN, which an HR_ADMIN-only gate then rejects.
...
- **3.1 Onboarding record shape:** **Job Number has no format rule** — a plain number (`1001`) or a text + number combination (`AIGH1002`, `EMP2004`) are both valid; only uniqueness is enforced, and the retired `AIGH-XXXX` pattern is not required. **Full Name is derived, never typed** — it is recomputed from First + Middle + Last on every keystroke and written into a read-only field (an omitted Middle Name leaves no double space). Fields after Job Number, in this order: Job Title, File No., Rank/Grade, Nationality, Job Post (Location Assignment) — City, Actual Work Place, Specialty, **Contract Start** and **Contract End** (both required, Gregorian picker with live Umm al-Qura Hijri conversion, both calendars stored), Marital Status (Single/Married/Others), Salary in Saudi Riyals. **The form opens on Nursing Unit = `Unassigned` and Position = `SN` — Staff Nurse.** `Unassigned` is a sentinel (`UNASSIGNED_UNIT_ID = 0`), deliberately *not* a row in `NURSING_UNITS`: adding it to the directory would break the seeded baseline of 47 units / 582 beds that the specification asserts and `verify_integration.py` F-01 machine-checks. `addEmployee` skips the unit lookup for the sentinel and still rejects any other unknown unit id. Demo employees carry plain job numbers `1001`–`4008`. **File No. has no format rule either** — the demo values are plain `1001`–`4008` with the retired `F-` prefix gone from both the seed data and the onboarding form placeholder, and nothing in the store constrains the shape. Because the store is persisted to `localStorage` (`aigh-workforce-storage`), seed edits alone never reach a browser that already loaded the app, so `store.tsx` ships `version: STORE_VERSION` plus a `migrate` step (`normalizePersistedEmployees`) that strips a leading `F-` or `AIGH-` on rehydrate — an existing session is migrated rather than replayed, and any other shape HR typed is left untouched. Verified by `npm run verify:employee-fields` (assertions against the real store + Hijri module, including 8 independently published Umm al-Qura anchor dates).
- **4.x Contract Copy [PDF] attachment:** Create Contract requires the signed contract copy. **The filter is one shared rule** — `checkContractCopyCandidate` in `src/lib/contracts.ts`, called by both the picker and `store.attachContractCopy`, so the screen and the write path cannot drift apart. It verifies the extension, the declared content type **and the `%PDF-` magic bytes** (§5.3.2 content-type verification, so a file merely renamed to `.pdf` is refused), enforces a 10 MB cap and rejects an empty file. The picker reads the first five bytes with `file.slice(0, 5).arrayBuffer()` and refuses **at pick time**, because the `accept` attribute is only a hint — every browser lets the user switch the dialog to "All files" — so a name-only filter would have let a renamed non-PDF through and only failed after HR had filled in the whole form. The store re-verifies the full bytes on write. Every upload appends a **new version** — historical bytes are never overwritten (§5.3.1) — and each is audited as `CONTRACT_COPY_ATTACHED`. `getContractCopyBytes` refuses anything whose scan status is not `CLEAN`, so an unscanned or infected copy is never handed out (§5.3.2). Attachments are HR Admin scoped (§4.2); Supervisor and Employee get a reduced read view with no evidence downloads. The **bytes live outside the zustand store** on purpose: `partialize` persists `contracts` to localStorage, so storing them on the row would push a base64 PDF into a ~5 MB quota on the first upload. Only metadata plus a `storageKey` is persisted, mirroring the real V41 secure vault.
- **4.x Contract guards in the store, not the screen:** `addContract` and `updateContract` reject a contract whose employee does not exist or is soft-deleted (`EMPLOYEE_NOT_FOUND`), reject a period whose end is on or before its start, and reject an `Approved`/`Active` period that overlaps another `Approved`/`Active` period for the same employee (`CONTRACT_PERIOD_OVERLAP`). `Draft`/`PendingApproval` provide no coverage, so they may sit on an existing period until approved. The rules live in `src/lib/contracts.ts` (`providesCoverage`, `periodsOverlap`, `latestContractFor`, `renewalPeriodAfter`) and mirror the DB exclusion constraint `GiST daterange && WHERE status IN (Approved,Active)`, so they hold no matter which screen calls them. **Create Contract appends the dates entered at Onboarding:** picking an employee shows that period (Gregorian + Hijri) and prefills the pickers with a renewal starting the day after the previous end, running the same length.
- **3.1.1 Position Assignment — HR Admin only (spec §8.1):** the Position Directory screen carries a **Position Assignment** action that sets an existing employee's position. Writing `employees.position` is an **Employee Master source field**, which §8.1 grants to HR Admin ("Maintain source fields within scope"); Supervisor/Scheduler holds an assigned-unit read view with private fields suppressed and Employee holds own profile and phone only, so neither may assign. The check lives in `store.assignPosition`, not in the screen, so it holds for every caller, and it also refuses a soft-deleted employee (`EMPLOYEE_NOT_FOUND`), an unknown or deprecated position (`POSITION_NOT_FOUND` / `POSITION_NOT_ACTIVE`) and a no-op re-assignment (`POSITION_UNCHANGED`). **§8.2 — the title never auto-confers a role:** DON, DEPUTY_DON, ADMIN, NS and ACTING_HEAD keep staff self-service until HR separately grants an elevated role and scope, so the action grants nothing and writes `authRoleGranted: null` into the audit entry to prove it.
